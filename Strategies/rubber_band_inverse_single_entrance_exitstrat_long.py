    ...
    for i in range(max_periods, len(df_data)):
        ...
        if condition is False and df_data.z_distance_mean[i] > z:
            ...
            if i + 1 < len(df_data):
                ...
                stoploss = buy_price - (buy_price * stop_loss)
                tf = buy_price + (buy_price * take_profit)
                # p_size_contracts = np.round((capital * 0.01) / (buy_price * par_stop_loss * CONTRACT_SIZE))
                p_size_contracts = 1
                margin = 500
...
def optimizer(iterator: list):
    ...
    all_data_optimizer.to_csv(f'{folder}results/{sample}_optimizer_{strategy_name}_long.csv')
...
def graphics():
    ...
    show(g)
    output_file(f'{folder}graphic_validation/{strategy_name}_long_graphic_validation.html', mode='inline')
    save(g)
...
if __name__ == '__main__':
    ...
    if sample == 'IS1':
        ...
        trades_fix.to_csv(f'{folder}results/{strategy_name}_trades_long_in.csv', index=False)
        ws.to_csv(f'{folder}results/{strategy_name}_long_windows_summary.csv', index=False)

    else:
        try:
            ws_oss = pd.read_csv(f'database_{strategy_name}_long_windows_summary.csv')
            ws = ws.append(ws_oss)
        ...
        trades_fix.to_csv(f'{folder}results/{strategy_name}_long_trades_long_out.csv', index=False)
